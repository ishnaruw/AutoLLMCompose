#!/usr/bin/env python3
"""
Recalculate composition-level QoS scores for existing AutoLLMCompose runs and refresh
all dashboard-facing composition artifacts.

What this updates for each run:
  - evaluation/query_<qid>_composition_qos_eval_rows.json
  - evaluation/query_<qid>_composition_qos_eval_summary.json
  - evaluation/query_<qid>_composition_qos_eval.xlsx
...
It does not rerun decomposition, retrieval, functional refinement, ranking, selection, or planning.
It calls src.eval.composition_qos_eval.evaluate_composition_qos() using your current source code.
"""
...
def discover_runs(target: Path, recursive: bool = False) -> List[Path]:
    ...
    if recursive:
        runs = [p for p in target.rglob("*") if is_run_dir(p)]
    else:
        runs = [p for p in target.iterdir() if is_run_dir(p)]
...
def backup_artifacts(run_dir: Path, query_id: str, run_stamp: str) -> Optional[Path]:
    backup_dir = run_dir / "evaluation" / f"recalc_backup_{run_stamp}"
    copied = 0
...
def patch_summary_file(summary_path: Path, run_dir: Path, project_root: Path) -> Dict[str, Any]:
    """Make summary paths stable and relative to the run where useful."""
    ...
    for key in path_keys:
        ...
        if not p.is_absolute():
            # Evaluator normally writes absolute paths. If not, resolve relative to run_dir first.
            candidate = (run_dir / p).resolve()
            p = candidate if candidate.exists() else (project_root / p).resolve()
        summary[key] = rel_to(p, run_dir)
    summary["composition_qos_recalculated_at_utc"] = utc_now()
    write_json(summary_path, summary)
    return summary


def scores_by_mode_from_rows(rows: List[Dict[str, Any]]) -> Dict[str, Dict[str, Any]]:
...
def patch_evaluation_result(run_dir: Path, result_paths: Dict[str, Path], summary: Dict[str, Any]) -> None:
    ...
    if isinstance(rows, list):
        payload["composition_scores_by_mode"] = scores_by_mode_from_rows(rows)

    score_summary = summary.get("score_summary") if isinstance(summary.get("score_summary"), dict) else {}
    if score_summary:
        # Convenience aliases for dashboards/scripts that do not inspect nested score_summary.
    ...
    payload["composition_qos_recalculated_at_utc"] = utc_now()
    payload["composition_qos_recalculation_source"] = "src.eval.composition_qos_eval.evaluate_composition_qos"
    payload["composition_qos_recalculation_note"] = (
        "Recomputed from saved planner/selection artifacts using the current composition evaluator."
    )
...
def patch_meta(run_dir: Path, result_paths: Dict[str, Path], summary: Dict[str, Any]) -> None:
    ...
    meta = read_json(path, {})
    timing = meta.setdefault("timing", {}) if isinstance(meta.get("timing", {}), dict) else {}
    meta["timing"] = timing
    stages = timing.setdefault("stages", {}) if isinstance(timing.get("stages", {}), dict) else {}
    timing["stages"] = stages
    stage = stages.setdefault("composition_qos_evaluation", {})
    ...
    meta["composition_qos_eval_recalculated_at_utc"] = utc_now()
    write_json(path, meta)


def append_log(run_dir: Path, summary: Dict[str, Any]) -> None:
    score_summary = summary.get("score_summary") if isinstance(summary.get("score_summary"), dict) else {}
    best = score_summary.get("best_modes_by_qos_adjusted_composition_score") or summary.get(
        "best_modes_by_qos_adjusted_composition_score"
    )
    if isinstance(best, list):
        best_text = ",".join(str(x) for x in best)
    else:
        best_text = str(best or "unknown")
    line = (
        f"[{utc_now()}] composition QoS artifacts recalculated; "
...
def verify_rows(result_paths: Dict[str, Path]) -> Dict[str, Any]:
    rows = read_json(result_paths["rows_json"], [])
    if not isinstance(rows, list):
        raise ValueError(f"Rows JSON is not a list: {result_paths['rows_json']}")
    found_modes = [str(row.get("Mode")) for row in rows if row.get("Mode")]
    score_map = scores_by_mode_from_rows(rows)
    return {
        "row_count": len(rows),
        "modes": ",".join(found_modes),
...
def run_one(
    ...
    if not candidate_rows.exists():
        raise FileNotFoundError(f"Missing candidate API ranking rows: {candidate_rows}")
    ...
    backup_dir = backup_artifacts(run_dir, query_id, run_stamp) if do_backup else None
    ...
    if not no_run_log:
        append_log(run_dir, summary)

    verification = verify_rows(result_paths)
    row = {
        "status": "updated",
        "run_name": run_dir.name,
        "query_id": query_id,
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Recalculate and refresh composition QoS artifacts for existing AutoLLMCompose runs."
    )
    parser.add_argument("--runs-dir", required=True, type=Path, help="Parent directory containing qXX_* run folders, or one run folder.")
    parser.add_argument("--project-root", type=Path, default=Path.cwd(), help="Project root containing src/. Default: cwd.")
    parser.add_argument("--recursive", action="store_true", help="Search recursively for run folders.")
    parser.add_argument("--only", nargs="*", help="Optional query IDs or run folder names to process, e.g. q06 q18_20260525T083005")
    parser.add_argument("--dry-run", action="store_true", help="Show runs that would be processed without writing files.")
    parser.add_argument("--no-backup", action="store_true", help="Do not back up old composition artifacts before overwriting.")
    parser.add_argument("--no-run-log", action="store_true", help="Do not append a recalculation note to run.log.")
    ...
    for run_dir in runs:
        try:
            row = run_one(
                ...
                no_run_log=args.no_run_log,
            )
            print(
                f"[{row['status']}] {row['run_name']} "
                f"hybrid={row.get('qos_hybrid_score')} pure={row.get('qos_pure_llm_score')} no_qos={row.get('no_qos_score')} topsis={row.get('qos_topsis_score')}"
            )
    ...
    if not args.dry_run:
        ...
        report_name = args.report_name or f"composition_score_refresh_report_{run_stamp}.json"
        report_path = report_base / report_name
        write_report(report_path, report_rows)
        latest_path = report_base / "composition_score_refresh_report_latest.json"
        ...
        print("\nStreamlit note: the dashboard caches composition reports. After this script finishes, click 'Reload reports' / 'Reload visualization data', or restart Streamlit if the old values still appear.")
    else:
