...
2) QoS guidance derived from code-computed QoS scoring
...
- topsis_rank = code-computed QoS rank, where 1 is best
- topsis_score = code-computed QoS score, where higher is better
...
- Prioritize QoS over functionality: APIs with better topsis_rank (lower number) should be ranked higher, even if functionally less direct.
- Among APIs with similar QoS ranks, prefer better functional suitability.
- Do not rank a functionally relevant API with a much worse topsis_rank above another functionally relevant API with a much better topsis_rank.
